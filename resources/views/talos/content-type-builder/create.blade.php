...
<div class="max-w-2xl" x-data="{ kind: '{{ request('kind', 'collectionType') }}' }">
    <form action="{{ route('talos.content-type-builder.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-6">
            <h2 class="text-slate-800 font-semibold mb-4">Select a type</h2>
            <div class="grid grid-cols-2 gap-4">
                <label class="cursor-pointer">
                    <input type="radio" name="kind" value="collectionType" x-model="kind" class="sr-only" />
                    <div :class="kind === 'collectionType' ? 'border-blue-500 bg-blue-50' : 'border-slate-300 hover:border-slate-300'"
                        ...
                        <h3 class="text-slate-800 font-semibold">Collection Type</h3>
                ...
                <label class="cursor-pointer">
                    ...
                    <div :class="kind === 'singleType' ? 'border-violet-500 bg-violet-50' : 'border-slate-300 hover:border-slate-300'"
                         class="border-2 rounded-xl p-4 transition-colors">
                        <div class="w-10 h-10 bg-violet-50 rounded-lg flex items-center justify-center mb-3">
                            <svg class="w-5 h-5 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        </div>
                        <h3 class="text-slate-800 font-semibold">Single Type</h3>
                        <p class="text-slate-400 text-xs mt-1">
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
            <h2 class="text-slate-800 font-semibold">Basic settings</h2>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-500 mb-1.5">API ID (singular) <span class="text-red-600">*</span></label>
                    ...
                    <p class="text-slate-400 text-xs mt-1">lowercase, no spaces</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-500 mb-1.5">API ID (plural) <span class="text-red-600">*</span></label>
            ...
            <div class="flex items-center justify-between p-4 bg-slate-100 rounded-lg">
                <div>
                    <p class="text-sm font-medium text-slate-800">Draft & Publish</p>
                    <p class="text-xs text-slate-400">Entries can be saved as draft before publishing</p>
                </div>
            ...
            <div class="flex items-center justify-between p-4 bg-slate-100 rounded-lg">
                <div>
                    <p class="text-sm font-medium text-slate-800">Internationalization (i18n)</p>
                    <p class="text-xs text-slate-400">Allow entries in multiple languages</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('talos.content-type-builder.index') }}"
               class="px-4 py-2 text-slate-500 hover:text-slate-900 text-sm">Cancel</a>
            <button type="submit"
