...
<div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            @foreach($contentTypes as $type)
                @if($type['kind'] === 'collectionType')
                    <div class="bg-gray-900 border border-gray-800 rounded-xl p-5 group hover:border-gray-700 transition-colors">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div class="flex items-center gap-1">
                                @if($type['options']['draftAndPublish'] ?? false)
                                    <span class="text-xs bg-green-900/40 text-green-400 border border-green-800 px-2 py-0.5 rounded">D&P</span>
                                @endif
                        ...
                        <h3 class="text-white font-semibold">{{ $type['info']['displayName'] }}</h3>
                        <p class="text-gray-500 text-xs mt-0.5">{{ $type['info']['pluralName'] }}</p>
                        <p class="text-gray-600 text-xs mt-2">{{ count($type['attributes'] ?? []) }} field(s)</p>
                        <div class="flex gap-2 mt-4">
                            <a href="{{ route('talos.content-type-builder.edit', ['uid' => $type['__uid']]) }}"
                               class="flex-1 text-center py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded text-xs font-medium transition-colors">
                                Edit Schema
                            </a>
                            <a href="{{ route('talos.content.index', ['uid' => $type['__uid']]) }}"
                               class="flex-1 text-center py-1.5 bg-blue-900/40 hover:bg-blue-900/60 text-blue-400 rounded text-xs font-medium transition-colors">
                                Manage Content
                            </a>
                        </div>
                    </div>
            ...
            <a href="{{ route('talos.content-type-builder.create') }}"
               class="border-2 border-dashed border-gray-800 hover:border-gray-600 rounded-xl p-5 flex flex-col items-center justify-center gap-3 text-gray-600 hover:text-gray-400 transition-colors min-h-[160px]">
            ...
            @foreach($contentTypes as $type)
                @if($type['kind'] === 'singleType')
                    <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
                        ...
                        <h3 class="text-white font-semibold">{{ $type['info']['displayName'] }}</h3>
                        <p class="text-gray-500 text-xs mt-0.5">{{ $type['info']['singularName'] }}</p>
                        <div class="flex gap-2 mt-4">
                            <a href="{{ route('talos.content-type-builder.edit', ['uid' => $type['__uid']]) }}"
                               class="flex-1 text-center py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded text-xs font-medium">
                                Edit Schema
                            </a>
                        </div>
                    </div>
    ...
    @if(count($components) > 0)
        <div>
            <h2 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Components</h2>
            <div class="bg-gray-900 border border-gray-800 rounded-xl divide-y divide-gray-800">
                @foreach($components as $component)
                    <a href="{{ route('talos.components.edit', ['uid' => $component['__uid']]) }}"
                       class="flex items-center justify-between px-5 py-3.5 hover:bg-gray-800 transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 bg-yellow-900/40 rounded-lg flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ $component['__category'] }} · {{ count($component['attributes'] ?? []) }} fields</p>
                            </div>
                        </div>
