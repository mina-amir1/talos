@php
    $selectedEvent  = old('event',            $rule?->event ?? '');
    ...
    $eventsOpts     = collect($events)->map(fn($l, $v) => ['v' => $v, 'l' => $l])->values()->all();
...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <div class="relative"
                 x-data='notifEventDrop(@json($selectedEvent), @json($eventsOpts))'
                 @click.outside="open = false">
                <input type="hidden" name="event" :value="value">
                <button type="button" @click="open = !open"
                    ...
                    <span class="truncate"
                          ...
                          x-text="label || '— select event —'"></span>
                ...
                <div x-show="open" x-cloak
                     class="absolute z-50 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg py-1 overflow-hidden">
                    <template x-for="o in opts" :key="o.v">
                        <button type="button" @click="value = o.v; open = false"
...
function notifEventDrop(initial, opts) {
    return {
        open:  false,
        value: initial,
        opts:  opts,
