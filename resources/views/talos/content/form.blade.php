...
@php
    ...
    // Serialised media list for Alpine store seed (avoid complex expression inside @json)
                                ...
                                <div x-data="{ ids: {{ json_encode($mInitIds) }}, show: false }">
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-3" x-show="ids.length > 0">
                                        <template x-for="_mi in $store._mlib.items.filter(i => ids.includes(i.id))" :key="_mi.id">
                                            <div class="relative group/thumb flex-shrink-0">
                                                <template x-if="_mi.isImage"><img :src="_mi.url" class="h-20 w-20 object-cover rounded-lg border border-slate-300"></template>
                                                <template x-if="!_mi.isImage"><div class="h-20 w-20 bg-slate-100 rounded-lg border border-slate-300 flex items-center justify-center"><span class="text-xs text-slate-400 font-mono uppercase" x-text="_mi.ext.toUpperCase()"></span></div></template>
                                    ...
                                    <div x-show="show" x-cloak
                                        ...
                                        <div class="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col">
                                            ...
                                            <div class="flex-1 min-h-0 flex overflow-hidden"><div class="w-48 border-r border-slate-200 flex-shrink-0 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto py-2"><button type="button" @click="$store._mlib.folder = null" :class="$store._mlib.folder === null ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm transition-colors">All media</button><template x-for="_mf in $store._mlib.folders" :key="_mf"><button type="button" @click="$store._mlib.folder = _mf" :class="_mf === $store._mlib.folder ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm truncate transition-colors flex items-center gap-2"><svg class="w-3.5 h-3.5 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg><span x-text="_mf.split('/').pop()"></span></button></template></div><div class="border-t border-slate-200 p-2" x-data="{ _nf: '', _nfOpen: false }"><div x-show="!_nfOpen"><button type="button" @click="_nfOpen = true" class="w-full flex items-center gap-1 px-2 py-1.5 text-xs text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded transition-colors"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> New folder</button></div><div x-show="_nfOpen" class="flex gap-1"><input type="text" x-model="_nf" @keydown.enter.prevent="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" @keydown.escape="_nfOpen=false; _nf=''" placeholder="folder-name" class="flex-1 min-w-0 px-2 py-1 text-xs border border-slate-300 rounded bg-white focus:outline-none focus:border-blue-400"><button type="button" @click="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></button><button type="button" @click="_nfOpen=false; _nf=''" class="px-2 py-1 text-slate-400 hover:text-slate-600 text-xs rounded hover:bg-slate-100"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div></div><div class="flex-1 min-h-0 overflow-y-auto"><div class="p-4 grid grid-cols-3 gap-4">
                                                <template x-for="_mi in $store._mlib.items" :key="_mi.id">
                                                    <button type="button"
                                                        ...
                                                        <div x-show="ids.includes(_mi.id)" class="absolute top-1.5 right-1.5 w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center pointer-events-none">
                                                            <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <label class="cursor-pointer" :class="$store._mlib.uploading && 'pointer-events-none'"><div class="flex items-center gap-1.5 text-sm" :class="$store._mlib.uploading ? 'text-blue-400' : 'text-blue-600 hover:text-blue-700'"><template x-if="!$store._mlib.uploading"><svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg></template><template x-if="$store._mlib.uploading"><svg class="w-4 h-4 flex-shrink-0 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></template><span x-text="$store._mlib.uploading ? $store._mlib.uploadProgress + '%' : 'Upload'"></span></div><div x-show="$store._mlib.uploading" class="mt-1 h-1 w-24 bg-slate-200 rounded-full overflow-hidden"><div class="h-full bg-blue-500 rounded-full transition-all duration-150" :style="'width:'+$store._mlib.uploadProgress+'%'"></div></div><input type="file" accept="image/*,video/*,application/pdf" class="sr-only" :disabled="$store._mlib.uploading" @change="if($store._mlib.uploading||!$event.target.files[0])return;$store._mlib.upload($event.target.files[0]).then(()=>{$event.target.value=''}).catch(()=>{$event.target.value=''})"></label>
                                                                    ...
                                                                    <div x-data="{ _mids: (() => { try { const v = row['{{ $subName }}']; return Array.isArray(v) ? v : (v ? JSON.parse(v) : []); } catch(e) { return []; } })(), _mshow: false }"
                                                                        ...
                                                                        <div x-show="_mshow" x-cloak class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6"
                                                                            ...
                                                                            <div class="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col">
                                                                                ...
                                                                                <div class="flex-1 min-h-0 flex overflow-hidden"><div class="w-48 border-r border-slate-200 flex-shrink-0 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto py-2"><button type="button" @click="$store._mlib.folder = null" :class="$store._mlib.folder === null ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm transition-colors">All media</button><template x-for="_mf in $store._mlib.folders" :key="_mf"><button type="button" @click="$store._mlib.folder = _mf" :class="_mf === $store._mlib.folder ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm truncate transition-colors flex items-center gap-2"><svg class="w-3.5 h-3.5 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg><span x-text="_mf.split('/').pop()"></span></button></template></div><div class="border-t border-slate-200 p-2" x-data="{ _nf: '', _nfOpen: false }"><div x-show="!_nfOpen"><button type="button" @click="_nfOpen = true" class="w-full flex items-center gap-1 px-2 py-1.5 text-xs text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded transition-colors"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> New folder</button></div><div x-show="_nfOpen" class="flex gap-1"><input type="text" x-model="_nf" @keydown.enter.prevent="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" @keydown.escape="_nfOpen=false; _nf=''" placeholder="folder-name" class="flex-1 min-w-0 px-2 py-1 text-xs border border-slate-300 rounded bg-white focus:outline-none focus:border-blue-400"><button type="button" @click="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></button><button type="button" @click="_nfOpen=false; _nf=''" class="px-2 py-1 text-slate-400 hover:text-slate-600 text-xs rounded hover:bg-slate-100"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div></div><div class="flex-1 min-h-0 overflow-y-auto"><div class="p-4 grid grid-cols-3 gap-4">
                                                                                    <template x-for="_mi in $store._mlib.items" :key="_mi.id">
                                                                                        <button type="button" @click="_mids.includes(_mi.id) ? _mids = _mids.filter(id => id !== _mi.id) : _mids.push(_mi.id); talos.markDirty()"
                                                                                                x-show="$store._mlib.folder===null||_mi.folder===$store._mlib.folder"
                                                                    ...
                                                                    <div x-data="{ _mid: row['{{ $subName }}'] ? parseInt(row['{{ $subName }}']) : null, _mshow: false }"
                                                                        ...
                                                                        <div x-show="_mshow" x-cloak class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6"
                                                                            ...
                                                                            <div class="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col">
                                                                                ...
                                                                                <div class="flex-1 min-h-0 flex overflow-hidden"><div class="w-48 border-r border-slate-200 flex-shrink-0 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto py-2"><button type="button" @click="$store._mlib.folder = null" :class="$store._mlib.folder === null ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm transition-colors">All media</button><template x-for="_mf in $store._mlib.folders" :key="_mf"><button type="button" @click="$store._mlib.folder = _mf" :class="_mf === $store._mlib.folder ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm truncate transition-colors flex items-center gap-2"><svg class="w-3.5 h-3.5 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg><span x-text="_mf.split('/').pop()"></span></button></template></div><div class="border-t border-slate-200 p-2" x-data="{ _nf: '', _nfOpen: false }"><div x-show="!_nfOpen"><button type="button" @click="_nfOpen = true" class="w-full flex items-center gap-1 px-2 py-1.5 text-xs text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded transition-colors"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> New folder</button></div><div x-show="_nfOpen" class="flex gap-1"><input type="text" x-model="_nf" @keydown.enter.prevent="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" @keydown.escape="_nfOpen=false; _nf=''" placeholder="folder-name" class="flex-1 min-w-0 px-2 py-1 text-xs border border-slate-300 rounded bg-white focus:outline-none focus:border-blue-400"><button type="button" @click="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></button><button type="button" @click="_nfOpen=false; _nf=''" class="px-2 py-1 text-slate-400 hover:text-slate-600 text-xs rounded hover:bg-slate-100"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div></div><div class="flex-1 min-h-0 overflow-y-auto"><div class="p-4 grid grid-cols-3 gap-4">
                                                                                    <template x-for="_mi in $store._mlib.items" :key="_mi.id">
                                                                                        <button type="button" @click="_mid = _mi.id; _mshow = false; talos.markDirty()"
                                                                                                x-show="$store._mlib.folder===null||_mi.folder===$store._mlib.folder"
                                                                                                        ...
                                                                                                        @elseif($nnField['type'] === 'media')
                                                                                                            ...
                                                                                                            <div x-data="{ _mids: (() => { try { const v = nr['{{ $nnName }}']; return Array.isArray(v) ? v : (v ? JSON.parse(v) : []); } catch(e) { return []; } })(), _mshow: false }"
                                                                                                                ...
                                                                                                                <div x-show="_mids.length > 0" class="flex flex-wrap gap-1.5 mb-2">
                                                                                                                    <template x-for="_mi in $store._mlib.items.filter(i => _mids.includes(i.id))" :key="_mi.id">
                                                                                                                        <div class="relative group">
                                                                                                                            <template x-if="_mi.isImage"><img :src="_mi.url" class="h-12 w-12 object-cover rounded"></template>
                                                                                                                            <template x-if="!_mi.isImage"><div class="h-12 w-12 bg-slate-100 flex items-center justify-center text-slate-500 text-xs rounded" x-text="_mi.ext"></div></template>
                                                                                                                            <button type="button" @click="_mids = _mids.filter(id => id !== _mi.id); talos.markDirty()" class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white rounded-full text-xs hidden group-hover:flex items-center justify-center">✕</button>
                                                                                                                        </div>
                                                                                                                    </template>
                                                                                                                </div>
                                                                                                                <button type="button" @click="$store._mlib.refresh(); _mshow = true" class="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded text-xs font-medium transition-colors">
                                                                                                                ...
                                                                                                                <div x-show="_mshow" x-cloak class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6" @keydown.escape.window="_mshow = false">
                                                                                                                    <div class="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col">
                                                                                                                        ...
                                                                                                                        <div class="flex-1 min-h-0 flex overflow-hidden"><div class="w-48 border-r border-slate-200 flex-shrink-0 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto py-2"><button type="button" @click="$store._mlib.folder = null" :class="$store._mlib.folder === null ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm transition-colors">All media</button><template x-for="_mf in $store._mlib.folders" :key="_mf"><button type="button" @click="$store._mlib.folder = _mf" :class="_mf === $store._mlib.folder ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm truncate transition-colors flex items-center gap-2"><svg class="w-3.5 h-3.5 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg><span x-text="_mf.split('/').pop()"></span></button></template></div><div class="border-t border-slate-200 p-2" x-data="{ _nf: '', _nfOpen: false }"><div x-show="!_nfOpen"><button type="button" @click="_nfOpen = true" class="w-full flex items-center gap-1 px-2 py-1.5 text-xs text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded transition-colors"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> New folder</button></div><div x-show="_nfOpen" class="flex gap-1"><input type="text" x-model="_nf" @keydown.enter.prevent="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" @keydown.escape="_nfOpen=false; _nf=''" placeholder="folder-name" class="flex-1 min-w-0 px-2 py-1 text-xs border border-slate-300 rounded bg-white focus:outline-none focus:border-blue-400"><button type="button" @click="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></button><button type="button" @click="_nfOpen=false; _nf=''" class="px-2 py-1 text-slate-400 hover:text-slate-600 text-xs rounded hover:bg-slate-100"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div></div><div class="flex-1 min-h-0 overflow-y-auto"><div class="p-4 grid grid-cols-3 gap-4">
                                                                                                                            <template x-for="_mi in $store._mlib.items" :key="_mi.id">
                                                                                                                                <button type="button" @click="_mids.includes(_mi.id) ? _mids = _mids.filter(id => id !== _mi.id) : _mids.push(_mi.id); talos.markDirty()" x-show="$store._mlib.folder===null||_mi.folder===$store._mlib.folder" :class="_mids.includes(_mi.id) ? 'border-blue-500' : 'border-transparent'" class="rounded-lg overflow-hidden border-2 transition-colors hover:border-blue-500">
                                                                                                                                    <template x-if="_mi.isImage"><img :src="_mi.url" class="w-full h-36 object-cover"></template>
                                                                                                                                    <template x-if="!_mi.isImage"><div class="w-full h-36 bg-slate-100 flex items-center justify-center text-slate-500 text-xs" x-text="_mi.ext"></div></template>
                                                                                                                                    <p class="text-xs text-slate-500 p-1 truncate" x-text="_mi.name"></p>
                                                                                                                                </button>
                                                                                                            ...
                                                                                                            <div x-data="{ _mid: nr['{{ $nnName }}'] ? parseInt(nr['{{ $nnName }}']) : null, _mshow: false }"
                                                                                                                ...
                                                                                                                <div x-show="_mid" class="mb-2">
                                                                                                                    <template x-for="_mi in $store._mlib.items.filter(i => i.id === _mid)" :key="_mi.id">
                                                                                                                        <div>
                                                                                                                            <template x-if="_mi.isImage"><img :src="_mi.url" class="h-14 w-auto object-cover rounded"></template>
                                                                                                                            <template x-if="!_mi.isImage"><p class="text-xs text-slate-500" x-text="_mi.name"></p></template>
                                                                                                                        </div>
                                                                                                                    </template>
                                                                                                                </div>
                                                                                                                <button type="button" @click="$store._mlib.refresh(); _mshow = true" class="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded text-xs font-medium transition-colors">
                                                                                                                ...
                                                                                                                <div x-show="_mshow" x-cloak class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6" @keydown.escape.window="_mshow = false">
                                                                                                                    <div class="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[85vh] flex flex-col">
                                                                                                                        ...
                                                                                                                        <div class="flex-1 min-h-0 flex overflow-hidden"><div class="w-48 border-r border-slate-200 flex-shrink-0 flex flex-col overflow-hidden"><div class="flex-1 overflow-y-auto py-2"><button type="button" @click="$store._mlib.folder = null" :class="$store._mlib.folder === null ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm transition-colors">All media</button><template x-for="_mf in $store._mlib.folders" :key="_mf"><button type="button" @click="$store._mlib.folder = _mf" :class="_mf === $store._mlib.folder ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-4 py-2.5 text-sm truncate transition-colors flex items-center gap-2"><svg class="w-3.5 h-3.5 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg><span x-text="_mf.split('/').pop()"></span></button></template></div><div class="border-t border-slate-200 p-2" x-data="{ _nf: '', _nfOpen: false }"><div x-show="!_nfOpen"><button type="button" @click="_nfOpen = true" class="w-full flex items-center gap-1 px-2 py-1.5 text-xs text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded transition-colors"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> New folder</button></div><div x-show="_nfOpen" class="flex gap-1"><input type="text" x-model="_nf" @keydown.enter.prevent="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" @keydown.escape="_nfOpen=false; _nf=''" placeholder="folder-name" class="flex-1 min-w-0 px-2 py-1 text-xs border border-slate-300 rounded bg-white focus:outline-none focus:border-blue-400"><button type="button" @click="if(_nf.trim()) $store._mlib.createFolder(_nf.trim()).then(()=>{ _nf=''; _nfOpen=false; })" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg></button><button type="button" @click="_nfOpen=false; _nf=''" class="px-2 py-1 text-slate-400 hover:text-slate-600 text-xs rounded hover:bg-slate-100"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div></div><div class="flex-1 min-h-0 overflow-y-auto"><div class="p-4 grid grid-cols-3 gap-4">
                                                                                                                            <template x-for="_mi in $store._mlib.items" :key="_mi.id">
                                                                                                                                <button type="button" @click="_mid = _mi.id; _mshow = false; talos.markDirty()" x-show="$store._mlib.folder===null||_mi.folder===$store._mlib.folder" :class="_mid === _mi.id ? 'border-blue-500' : 'border-transparent'" class="rounded-lg overflow-hidden border-2 transition-colors hover:border-blue-500">
                                                                                                                                    <template x-if="_mi.isImage"><img :src="_mi.url" class="w-full h-36 object-cover"></template>
                                                                                                                                    <template x-if="!_mi.isImage"><div class="w-full h-36 bg-slate-100 flex items-center justify-center text-slate-500 text-xs" x-text="_mi.ext"></div></template>
                                                                                                                                    <p class="text-xs text-slate-500 p-1 truncate" x-text="_mi.name"></p>
                                                                                                                                </button>
                                            ...
                                            <div>
                                                ...
                                                @elseif($subField['type'] === 'richtext')
                                                    <div x-data="{ _ql: null }" x-init="
                                                        ...
                                                        const _iv = d['{{ $subName }}'];
                                                        if (_iv) _ql.clipboard.dangerouslyPasteHTML(_iv);
                                                        _ql.on('text-change', () => { const _h = _ql.root.innerHTML; d['{{ $subName }}'] = _h === '<p><br></p>' ? '' : _h; talos.markDirty(); });
                                                    "><div data-q></div></div>
                                                @elseif($subField['type'] === 'component')
                                                    @php
                                                        $nestedUid    = $subField['components'][0] ?? null;
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        @elseif($nnField['type'] === 'media')
                                                                            ...
                                                                            <div x-data="{ _mid: (d.{{ $subName }} ?? {})['{{ $nnName }}'] ? parseInt((d.{{ $subName }} ?? {})['{{ $nnName }}']) : null, _mshow: false }"
                                                                                ...
                                                                                <div x-show="_mid" class="mb-2">
                                                                                    <template x-for="_mi in $store._mlib.items.filter(i => i.id === _mid)" :key="_mi.id">
                                                                                        <div><template x-if="_mi.isImage"><img :src="_mi.url" class="h-14 w-auto object-cover rounded"></template><template x-if="!_mi.isImage"><p class="text-xs text-slate-500" x-text="_mi.name"></p></template></div>
                                                                                    </template>
                                                                                </div>
                                                                                <button type="button" @click="$store._mlib.refresh(); _mshow = true" class="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded text-xs font-medium transition-colors">
                                                                                    <span x-text="_mid ? 'Change' : 'Select from library'"></span>
                                                                            ...
                                                                            @endif
                                                                                        @elseif($nnField['type'] === 'richtext')
                                                                                            <div x-data="{ _ql: null }" x-init="
                                                                                                ...
                                                                                                if (_iv) _ql.clipboard.dangerouslyPasteHTML(_iv);
                                                                                                _ql.on('text-change', () => { const _h = _ql.root.innerHTML; const _o = (d['{{ $subName }}'] ??= {}); _o['{{ $nnName }}'] = _h === '<p><br></p>' ? '' : _h; talos.markDirty(); });
                                                                                            "><div data-q></div></div>
                                                                        @else
                                                                            <input type="text" x-model="(d.{{ $subName }} ??= {})['{{ $nnName }}']" class="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-800 text-xs focus:outline-none focus:border-blue-500">
                                                                        @endif
            ...
            return new Promise((resolve, reject) => {
                ...
                xhr.addEventListener('load', async () => {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        // If conversion is queued, refresh again once it likely finishes
